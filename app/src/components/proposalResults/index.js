    ...
    return (
      <div className="results">
        ...
        <div className="results-bar">
          {!!results.for && (
            <span
              className={'for-bar ' + (results.for > results.against && 'winner')}
              style={getVoteBarWidth(results.for)}>
              <div>
                <span>{results.for}</span>
                <span className="vote-results-percentage">{Math.round(results.for / totalNumberOfVotes * 100)}%</span>
              </div>
            </span>
          )}
          {!!results.blank && (
            <span className="blank-bar" style={getVoteBarWidth(results.blank)}>
              <div>
                <span>{results.blank}</span>
                <span className="vote-results-percentage">{Math.round(results.blank / totalNumberOfVotes * 100)}%</span>
              </div>
            </span>
          )}
          {!!results.against && (
            <span
              className={'against-bar ' + (results.against > results.for && 'winner')}
              style={getVoteBarWidth(results.against)}>
              <div>
                <span>{results.against}</span>
